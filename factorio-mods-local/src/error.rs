...
pub enum ErrorKind {
	...
	#[error_chain(custom)]
	#[error_chain(display = |pattern, _| write!(f, "The pattern {} is invalid", pattern))]
	#[error_chain(cause = |_, err| err)]
	Pattern(String, ::globset::Error),

	/// The local Factorio installation could not be found.
	#[error_chain(custom)]
	#[error_chain(display = || write!(f, "The local Factorio installation could not be found"))]
	DataPath,
	...
	#[error_chain(custom)]
	#[error_chain(display = |_| write!(f, "Valid API credentials were not found in player-data.json"))]
